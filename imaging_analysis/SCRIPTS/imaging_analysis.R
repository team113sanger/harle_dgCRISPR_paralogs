...
# Remove proliferative unstained outliers - 
# Several low percentages stick out from two plates only as poorly stained (outliers from group so removed)
...
# Get total number of objects per well for all replicates 
# Note this is scaling one, for the objects (no. of cells)
# Think normalisation by cell number so the proportions are comparable across the whole plate.
...
Plot_percentage_non_prolif <- ggplot(Plot_order, aes(x = fct_inorder(Target), y = Percentage_Non_proliferative, fill = Group_Target)) + 
  ...
  labs(title='Percentage Not Proliferative', y = 'Percentage Not Proliferative', x='Target') +
  ylim(0,100)

Plot_percentage_prolif <- ggplot(Plot_order, aes(x = fct_inorder(Target), y = Percentage_Proliferative, fill = Group_Target)) + 
  geom_boxplot() +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), legend.position = 'none') + 
  labs(title = 'Percentage Proliferative Cells', y = 'Percentage Proliferative', x = 'Target') +
  ylim(0,100)

Plot_percentage_apoptotic <- ggplot(Plot_order, aes(x = fct_inorder(Target), y = Percentage_Apoptotic, fill = Group_Target)) + 
  geom_boxplot() +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), legend.position = 'none') + 
  labs(title = 'Percentage Apoptotic Cells', y = 'Percentage Apoptotic', x = 'Target') +
  ylim(0,100)

Plot_percentage_enlarged <- ggplot(Plot_order, aes( = fct_inorder(Target), y = Percentage_Enlarged, fill = Group_Target)) + 
  geom_boxplot() +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), legend.position = 'none') + 
  labs(title = 'Percentage Enlarged Cells', y = 'Percentage Enlarged', x = 'Target') + 
  ylim(0,15)
...
combined_percentage_classified_figure <- grid.arrange(Plot_percentage_non_prolif,
                                                      ...
                                                      Plot_percentage_enlarged, 
                                                      nrow = 2)

# Save combined plot to file 
ggsave(file.path(plate_directory, 'PLOTS', 'combined_percentage_classified_figure .png'), combined_percentage_classified_figure , dpi = 300, width = 12, height = 10)
...
Percentage_scaled <- Average_percentage_scaled |> 
                     group_by(Target) |> mutate(`Non Proliferative` = mean(Percentage_Non_proliferative),
                                                 `Proliferative` = mean(Percentage_Proliferative),
                                                 `Apoptotic` = mean(Percentage_Apoptotic),
                                                 `Enlarged` = mean(Percentage_Enlarged)) |>
  select(c("Target", "Group_Target", "Non Proliferative" , "Proliferative", "Apoptotic", "Enlarged")) |> 
  unique()

Percentage_scaled_pivot <- Percentage_scaled |> 
                           pivot_longer(c(`Non Proliferative`, `Proliferative`,  `Apoptotic`, `Enlarged`), 
                                        names_to = 'Cell Classification', values_to = 'Percentage') 
...
# Save file
ggsave(file.path(plate_directory, 'PLOTS', 'Barplot of Average Classified Cells Scaled Data.png'), Barplot_Average_Percentage , dpi = 300, width = 12, height = 10)

#Remove Clinical Targets - plot both ways, with and without clinical
Average_percentage_scaled_Top_Hits_only <- Average_percentage_scaled |> 
  filter(!Target %in% c("EAF1", "EAF2", "EAF1|EAF2",
                        "INTS6", "INTS6L", "INTS6|INTS6L", 
                        "TTC7A", "TTC7B", "TTC7A|TTC7B"))

colnames(Average_percentage_scaled_Top_Hits_only)
Percentage_scaled_Top_Hits_only<- Average_percentage_scaled_Top_Hits_only |> group_by(Target) |> 
  mutate(`Non Proliferative` = mean(Percentage_Non_proliferative),
        `Proliferative` = mean(Percentage_Proliferative),
        `Apoptotic` = mean(Percentage_Apoptotic),
        `Enlarged` = mean(Percentage_Enlarged)) |>
...
## Save file
ggsave(file.path(plate_directory, 'PLOTS', 'Barplot of Average Classified Cells Scaled Data Top Hits Only.png'), Barplot_Average_Percentage_Top_Hits_only , dpi = 300, width = 12, height = 10)

# Work out change in fold change for second figure - 
# Firstly plot this was all targets, then plot again removing the clinical targets
# Fold change for apoptosis
# Rationale - compare change in apoptosis to control only for each replicate to remove total staining batch affects
# Select only apoptosis and relevant columns
...
# Figure excludes the statistics